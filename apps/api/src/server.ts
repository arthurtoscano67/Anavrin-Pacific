...
function uniquePackageIds(packageIds: Array<string | null | undefined>) {
...
function resolveAvatarPackageIds(packageIdOverride?: string) {
  const configured =
    packageIdOverride?.trim()
      ? [packageIdOverride.trim()]
      : [
          apiConfig.AVATAR_PACKAGE_ID,
          process.env.VITE_AVATAR_PACKAGE_ID?.trim() || null,
          ...apiConfig.LEGACY_AVATAR_PACKAGE_IDS,
        ];

  const resolved = uniquePackageIds(configured);
