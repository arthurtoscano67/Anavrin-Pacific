...
function buildDefaultOperatorDescription(preset: ShooterCharacterPreset | null) {
  if (!preset) {
    return "Wallet-owned operator for the Pacific shooter runtime.";
  }

  return `${preset.tagline}. Bound to the ${preset.role} class in Pacific shooter matches.`;
}
...
function deriveExtendCardFromBackend(avatar: BackendOwnedAvatar): ExtendOperatorCard | null {
  ...
  return {
    ...
    previewUrl: buildPublicAssetUrl(previewBlobId),
...
function App() {
  ...
  const [phase, setPhase] = useState<Phase>(() =>
    resolveAppRoute(window.location.pathname, window.location.search) === "/create"
      ? "mint"
      : "home",
  );
  const [name, setName] = useState(SHOOTER_CHARACTER_PRESETS[0]?.label ?? "Pacific Operator");
  const [description, setDescription] = useState(() =>
    buildDefaultOperatorDescription(SHOOTER_CHARACTER_PRESETS[0] ?? null),
  );
  ...
  const mintReadiness = useMemo<MintReadinessItem[]>(
    () => [
      ...
      {
        id: "wallet-media",
        label: "Wallet image URL public",
        ready: publicAssetReady,
        detail: publicAssetReady
          ? `Wallet media resolves from ${webEnv.publicAssetBaseUrl}/preview/<blobId>.`
          : "Set VITE_PUBLIC_ASSET_BASE_URL to a public HTTPS Pacific preview gateway before minting wallet-visible media.",
      },
      {
        id: "runtime",
  ...
  const estimatedWalletPromptCount =
    4 + (sourceAssetFile ? 1 : 0) + 1 + (apiAvailable === false ? 0 : 1);
  ...
  useEffect(() => {
    const nextHref = buildQueryAppHref(phase === "home" ? "/" : "/create", {
      mode: selectedGameMode === "shooter" ? "shooter" : null,
    });
    const currentHref = `${window.location.pathname}${window.location.search}`;
    if (currentHref !== nextHref) {
      window.history.replaceState({}, "", nextHref);
    }
  }, [phase, selectedGameMode]);
  ...
  useEffect(() => {
    if (!selectedShooterPreset) {
      return;
    }

    if (!nameDirty || name.trim().length === 0) {
      setName(selectedShooterPreset.label);
    }

    if (!descriptionDirty || description.trim().length === 0) {
      setDescription(buildDefaultOperatorDescription(selectedShooterPreset));
    }
  }, [description, descriptionDirty, name, nameDirty, selectedShooterPreset]);
  ...
  const handlePublish = useCallback(async () => {
    ...
    try {
      ...
      const initialShooterStats = { ...shooterInitialStats };
      const publicPreviewUrl = buildPublicAssetUrl(previewUpload.blobId);
      const genericProjectUrl = resolveMintProjectUrl();
      const displayDescription = buildAvatarDisplayDescription(
        manifestDescription,
        initialShooterStats,
      );
      ...
      const profileUrl = buildAvatarProfileUrl(avatarObjectId);
      let profileLinked = false;
      try {
        updateMintStatus(
          "linking public profile",
          "Approve the metadata sync so the NFT opens the public Pacific profile with the current stats.",
        ...
        console.warn("Avatar metadata sync did not complete.", profileLinkError);
      }
  ...
  const resetMintIdentity = useCallback((preset: ShooterCharacterPreset | null) => {
    setNameDirty(false);
    setDescriptionDirty(false);
    setName(preset?.label ?? "Pacific Operator");
    setDescription(buildDefaultOperatorDescription(preset));
  }, []);
  ...
  const openMint = useCallback(() => {
    ...
    setRenewNotice(null);
    setRenewError(null);
    resetMintIdentity(selectedShooterPreset ?? SHOOTER_CHARACTER_PRESETS[0] ?? null);
  }, [resetMintIdentity, selectedShooterPreset]);
    ...
    if (phase === "identity") {
      return (
        <section className="phase-screen">
          ...
          <p className="section-copy">
            Give the operator a codename and a short story. This is the name that shows on the NFT.
          </p>
          <div className="form-stack">
            <label className="form-field">
              ...
              <input
                ...
                placeholder={selectedShooterPreset?.label ?? "Operator name"}
              />
            </label>
            <label className="form-field">
              ...
              <input
                ...
                placeholder={
                  selectedShooterPreset
                    ? buildDefaultOperatorDescription(selectedShooterPreset)
                    : "Set a short operator description."
                }
              />
            </label>
          </div>
          ...
          {publishState ? (
            <div className="summary-grid">
              ...
              <div className="summary-item">
                ...
                <strong>{publishState.profileLinked ? "Synced" : "Needs sync"}</strong>
              </div>
            </div>
          ) : null}
          {publishState && !publishState.profileLinked ? (
            <div className="notice-callout">
              The NFT minted successfully, but the follow-up metadata sync was skipped. Use the
              Play screen to sync the public profile link and live stats into the NFT.
            </div>
          ) : null}
          <div className="action-row">
            ...
            {publishState ? (
              <a className="secondary-button" href={publishState.profileUrl}>
                View Public Profile
              </a>
            ) : null}
          ...
          <section className="screen-hero screen-hero--mint">
            ...
            <div className="screen-hero-art">
              ...
              <div className="hero-art-caption">
                ...
                <strong>{selectedShooterPreset?.label ?? "Choose operator"}</strong>
