...
async function readManifestDescription(manifestBlobId: string) {
  const response = await fetch(`${webEnv.apiBaseUrl}/manifest/${encodeURIComponent(manifestBlobId)}`);
  if (!response.ok) {
    return null;
  }
...
export function UnityPage() {
  ...
  const syncSelectedAvatarToChain = useCallback(async () => {
    if (!selectedAvatar) {
      setSyncError("Choose a character before syncing the NFT.");
      return;
    }

    setSyncBusyLabel("Syncing NFT Stats");
    setSyncNotice(null);
    setSyncError(null);

    try {
      const onChain = await fetchOnChainAvatarMetadata(selectedAvatar.objectId);
      if (!onChain.objectType?.endsWith("::avatar::Avatar")) {
        throw new Error("This operator was minted with the legacy avatar contract and cannot sync live NFT stats.");
      }

      const manifestDescription =
        onChain.description ||
        (selectedAvatar.manifestBlobId
          ? await readManifestDescription(selectedAvatar.manifestBlobId)
          : null) ||
        `${selectedAvatar.name ?? "Pacific Operator"} playable shooter operator.`;
      const previewBlobId = onChain.previewBlobId || selectedAvatar.previewBlobId;
      const manifestBlobId = onChain.manifestBlobId || selectedAvatar.manifestBlobId;
      if (!previewBlobId || !manifestBlobId) {
        throw new Error("The NFT is missing preview or manifest metadata needed for an on-chain refresh.");
      }

      const nextPreviewUrl =
        hasPublicAssetGateway() ? buildPublicAssetUrl(previewBlobId) : onChain.previewUrl;
      if (!nextPreviewUrl) {
        throw new Error("The NFT does not have a usable preview URL yet.");
      }
      ...
      await updateAvatarObject(client, dAppKit, {
        ...
        displayDescription: buildAvatarDisplayDescription(
          manifestDescription,
          selectedAvatar.shooterStats,
        ),
        manifestBlobId,
        previewBlobId,
        previewUrl: nextPreviewUrl,
        projectUrl: buildAvatarProfileUrl(selectedAvatar.objectId),
      ...
      setSyncError(
        caught instanceof Error ? caught.message : "NFT stat sync failed.",
      );
    } finally {
    ...
    isRuntimeScreen ? (
      <div className="runtime-immersive-shell">
        <div className="runtime-immersive-topbar">
          <a className="secondary-button" href={buildQueryAppHref("/unity")}>
            Back To Characters
          </a>
                ...
                <div className="launch-stage-panel">
                  ...
                  <div className="action-row action-row--tight">
                    <button
                      ...
                      disabled={Boolean(syncBusyLabel)}
                      onClick={() => void syncSelectedAvatarToChain()}
                      type="button"
                    >
                      {syncBusyLabel ?? "Sync NFT Stats"}
                    </button>
                    {selectedProfileUrl ? (
                      <a className="secondary-button" href={selectedProfileUrl}>
                        View Profile
                      </a>
                    ) : null}
                  </div>
